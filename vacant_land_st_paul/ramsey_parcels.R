...
stpaul_revenue <- st_paul_24 %>%
  st_drop_geometry()%>%
  dplyr::summarise(total_revenue = sum(TOTAL_TAX, na.rm = TRUE))
...
st_paul_24$city_tax_est <- st_paul_24$TAX_CAPAC*st_paul_24$city_rate
...
n_vacant <- st_paul_24 %>% st_drop_geometry() %>% filter(.data[[flag_col]] == 1) %>% summarise(n = n()) %>% pull(n)
current_fee <- 2705
